...
#### 洪水填充 (Flood Fill)
為了讓 AI 具備「空間感知能力」，避免走入 $U$ 型或 $C$ 型的封閉死路，以此案例實作了基於 **廣度優先搜尋 (BFS)** 的洪水填充演算法。
演算法在 `game.py` 的 `get_accessible_area` 中實作，具體步驟如下：

1.  **碰撞預判**：
    * 輸入預測的下一步像素座標 (`point`)。
    * 若該點直接撞牆或撞到身體，直接回傳 `0` (代表無空間，必死)。

2.  **初始化與座標轉換**：
    * 將像素座標轉換為網格索引 (`Grid Index = Pixel // BLOCK_SIZE`)。
    * 設定 BFS 佇列 與已訪問的集合。
    * 將蛇的身體座標轉換為 **障礙物集合**。

3.  **設定安全閾值**：
    * 設定搜尋上限 `limit = len(self.snake) + 5`。
    * *目的*：我們不需要算出地圖的總面積，只要確定空間「比蛇身長一點點」就代表安全。這能大幅減少運算量。

4.  **BFS 擴散與計數 (BFS Loop)**：
    * 從 Queue 取出當前點，計數器 `count` 加 1。
    * **提早終止**：若 `count > limit`，代表空間充足，立即停止搜尋並回傳 `limit`。
    * **鄰居檢查**：檢查上下左右四個鄰居，若「不越界」、「未訪問」且「非障礙物」，則加入 Queue。

5.  **Agent 狀態特徵輸出 (Feature Extraction)**：
    * 在 `agent.py` 中，分別計算直走、右轉、左轉的可行區域 `area`，當作輸入層並輸入。
